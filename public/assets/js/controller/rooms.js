$(function(){
    ...
    //入住状态筛选
    $('#isCheckInSelect').change(function(){
	    pageNow = 1;
	    changTable(pageNow);
    })

    //清洁状态筛选
    $('#isCleanSelect').change(function(){
	    pageNow = 1;
	    changTable(pageNow);
    })
 ...
 */
    function changTable(pageNow){
		...
		var typeId = parseInt($('#typeSelect').val()); //已选房间类型id
		var isCheckIn = parseInt($('#isCheckInSelect').val()); //已选入住状态
		var isClean = parseInt($('#isCleanSelect').val()); //已选清洁状态
		...
		if(isCheckIn !== -1){
           paramObj.status = isCheckIn;
		}

		if(isClean !== -1){
           paramObj.cleanStatus = isClean;
		}
               ...
               setTimeout(function(){
                      ...
                      for(var i = 0; i < dataList.length; i++){
                         ...
                         if(itermDate.status == 1){
                            isCheckInDom = '<button type="button" class="btn btn-primary btn-xs">已入住</button>'
                         }else{
                            isCheckInDom = '<button type="button" class="btn btn-success btn-xs">空置</button>'
                         }

                         if(itermDate.cleanStatus == 1){
                            isCleanDom = '<button type="button" class="btn btn-success btn-xs">已打扫</button>'
                         }else{
                            isCleanDom = '<button type="button" class="btn btn-danger btn-xs">未打扫</button>'
                         }
               	         ...
               	         var itermDom = '<tr class="even pointer">\
                                             ...
                                             <td>'+ itermDate.number +'</td>\
                                             <td>'+ itermDate.RoomType.name +'</td>\
                                             <td>'+ isCheckInDom +'</td>\
                                             <td>'+ isCleanDom +'</td>\
