...
function changTable(pageNow){
    $('#loadingBox').show(); //显示loading加载动画
		...
		var tasktypeId = parseInt($('#taskTypeSelect').val()); //已选任务类型
		var status = parseInt($('#statusSelect').val()); //已选任务状态
		...
		if(tasktypeId !== -1){
           paramObj.tasktypeId = tasktypeId;
		}
               ...
               setTimeout(function(){
                      ...
                      for(var i = 0; i < dataList.length; i++){
                         ...
                         if(itermDate.status == 1){
                            statusDom = '<button type="button" class="btn btn-success btn-xs">已完成</button>'
                         }else{
                            statusDom = '<button type="button" class="btn btn-danger btn-xs">未完成</button>'
                         }
